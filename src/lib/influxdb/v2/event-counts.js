...
export async function storeEventCountV2() {
    try {
        ...
        globals.logger.error(
            `EVENT COUNT V2: Error saving event counts: ${globals.getErrorMessage(err)}`
        );
    }
}
...
export async function storeRejectedEventCountV2() {
    try {
        ...
        globals.logger.error(
            `REJECTED EVENT COUNT V2: Error saving rejected event counts: ${globals.getErrorMessage(err)}`
        );
    }
}
