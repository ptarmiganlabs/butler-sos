...
| Queue rate limit | `Butler-SOS.auditEvents.queue.rateLimit.*` | Throttle how fast Butler SOS writes to downstream destinations (InfluxDB, Parquet, QVD, JSON) | `429 Too Many Requests` with `outcome: "dropped"`, `reason: "Rate limit exceeded"` | `warn: AUDIT API: Dropped audit event due to queue rate limit ...` |
