...
| `user` | `payload.context.user` | When non-empty string | Full Qlik Sense user identity. |
| `userDirectory` | Parsed from `payload.context.user` | When parsing succeeds | Qlik Sense user directory. |
| `userId` | Parsed from `payload.context.user` | When parsing succeeds | Qlik Sense user ID. |
...
**Important**: In Grafana with InfluxQL data source, use `$__timeFrom` and `$__timeTo` macros for time filters. When using `GROUP BY`, `ORDER BY` must reference a tag in the `GROUP BY` clause (e.g., `ORDER BY "user"`).
...
SELECT "time", "eventType", "user", "appName", "objectId"
FROM audit_event
WHERE "time" >= $__timeFrom AND "time" <= $__timeTo
ORDER BY "time" DESC
LIMIT 100;
...
SELECT "time", "eventType", "user", "sheetName", "objectId", "durationMs"
FROM audit_event
...
SELECT "user", COUNT("user") AS "event_count"
FROM "audit_event"
WHERE "time" >= $__timeFrom AND "time" <= $__timeTo
GROUP BY "user"
ORDER BY "user";
...
SELECT "time", "user", "appName", "selectionDetails"
FROM "audit_event"
